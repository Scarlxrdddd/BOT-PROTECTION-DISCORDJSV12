...
module.exports = function (client) {
    ...
    client.on("guildMemberAdd", async member => {
      ...
      var textString4 = `Tu es le #${member.guild.memberCount}ème`;
      ...
      const welcomeembed = new Discord.MessageEmbed()
        .setColor("RANDOM")
        .setTimestamp()
        .setFooter("Nouveau Membre", member.guild.iconURL({ dynamic: true }))
        .setDescription(`**Bienvenue sur ${member.guild.name}!**
      Salut <@${member.id}> ! Vérifie toi !`)
        .setImage("attachment://welcome-image.png")
        .attachFiles(attachment);
      //define the welcome channel
      let welcomechannel = db.fetch(`welcommechannel_${member.guild.id}`)
      const channel = member.guild.channels.cache.filter(ch => ch.id === welcomechannel);
